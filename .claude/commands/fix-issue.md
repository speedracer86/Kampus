...
Note in particular which surface the change belongs to — `packages/engine` (pure math),
`packages/ui` (a design handoff: compose, don't restyle), `apps/web` (the Phase-0 slice), or
the legacy `index.html`. The React app and `index.html` are independent and share no code, so
a fix in one does not propagate to the other; say so if the issue actually needs both.
...
- How you'll verify it (see `CLAUDE.md` — `npm test && npm run build` always;
  `npm run test:e2e` as well if you touched `index.html`)
